...
// @downloadURL  https://raw.githubusercontent.com/joucho1209/HVEA/main/HV%20Equipment%20Assistant.js
// @author       joucho
...
let hvToastStyleElement = null;

function ensureToastStyle() {
  if (hvToastStyleElement?.isConnected || document.getElementById("hv-ms-toast-style")) return;
  hvToastStyleElement = document.createElement("style");
  hvToastStyleElement.id = "hv-ms-toast-style";
  hvToastStyleElement.textContent = `
    .hv-ms-toast {
      position: fixed;
      left: 50%;
      bottom: 28px;
  ...
  `;
  (document.head || document.documentElement).appendChild(hvToastStyleElement);
}
...
(function() {
  ...
  let gStatsPanelIndex = null;
  let gStatsPanelIndexRoot = null;

  function invalidateStatsPanelIndex() {
    gStatsPanelIndex = null;
    gStatsPanelIndexRoot = null;
  }

  function getStatsPanelIndex() {
    const root = document.getElementById('stats_scrollable');
    if (!root) return null;
    if (gStatsPanelIndex && gStatsPanelIndexRoot === root) return gStatsPanelIndex;
    gStatsPanelIndexRoot = root;
    gStatsPanelIndex = buildStatsPanelIndex(root);
    return gStatsPanelIndex;
  }
  ...
  const CHARM_EFFECT_CACHE = new Map();

  function getCharmEffect(type, size, weaponClass, level) {
    const cacheKey = `${type}|${size}|${weaponClass}|${Number(level) || 0}`;
    if (CHARM_EFFECT_CACHE.has(cacheKey)) return CHARM_EFFECT_CACHE.get(cacheKey);
    const def = CHARM_DEFS[type];
    const sizeSpec = def?.sizes?.[size];
    if (!sizeSpec) {
      const empty = Object.freeze(createZeroCharmEffect());
      CHARM_EFFECT_CACHE.set(cacheKey, empty);
      return empty;
    }
    ...
    const cached = Object.freeze(effect);
    CHARM_EFFECT_CACHE.set(cacheKey, cached);
    return cached;
  }
  ...
  function getCounterResistCharmBonusFromInfo(info) {
    const charm = Array.isArray(info)
      ? info.find(item => item?.type === 'penetrator')
      : null;
    if (!charm) return 0;
    return charm.size === 'greater'
      ? COUNTER_RESIST_CHARM_BONUS.greater
      : COUNTER_RESIST_CHARM_BONUS.lesser;
  }
  ...
  let charmInfoAjaxTimer = null;

  function pumpCharmInfoAjaxQueue() {
    if (!CHARM_INFO_AJAX_QUEUE.length ||
      charmInfoAjaxActive >= CHARM_INFO_AJAX_MAX) return;

    const elapsed = Date.now() - charmInfoAjaxLastStart;
    const wait = Math.max(0, CHARM_INFO_AJAX_INTERVAL - elapsed);
    if (wait > 0) {
      if (charmInfoAjaxTimer === null) {
        charmInfoAjaxTimer = window.setTimeout(() => {
          charmInfoAjaxTimer = null;
          pumpCharmInfoAjaxQueue();
        }, wait);
      }
      return;
    }

    const job = CHARM_INFO_AJAX_QUEUE.shift();
    charmInfoAjaxActive++;
    charmInfoAjaxLastStart = Date.now();
    let released = false;
    const release = () => {
      if (released) return;
      released = true;
      charmInfoAjaxActive = Math.max(0, charmInfoAjaxActive - 1);
      pumpCharmInfoAjaxQueue();
    };
  ...
  function enqueueCharmInfoAjax(job) {
    if (typeof job !== 'function') return;
    CHARM_INFO_AJAX_QUEUE.push(job);
    pumpCharmInfoAjaxQueue();
  }
  ...
  function getCounterResistCharmBonusForEquipment(eid) {
    ...
    async function waitForEquipmentCharmData(equipSlots, force) {
        ...
        results.forEach(({ slot, info }) => {
            ...
            if (charms.length) {
                CHARM_INFO_CACHE.set(equipmentId, charms);
                invalidateEquipmentDataCache();
                charmInfoCacheChanged = true;
            }
            slot.actualCharms = charms;
            const bonus = getCounterResistCharmBonusFromInfo(charms);
            if (Number(bonus) > 0) {
        ...
        if (charmInfoCacheChanged) persistCharmInfoCache();
    ...
    function invalidateEquipmentDataCache() {
    ...
    function cloneEquipmentSlots(slots) {
        return (slots || []).map(slot => ({
            ...slot,
            stats: Array.isArray(slot.stats) ? slot.stats.map(stat => ({ ...stat })) : [],
            charmInfo: Array.isArray(slot.charmInfo)
                ? slot.charmInfo.map(charm => ({ ...charm }))
                : slot.charmInfo,
            actualCharms: Array.isArray(slot.actualCharms)
                ? slot.actualCharms.map(charm => ({ ...charm }))
                : slot.actualCharms,
        }));
    }

    function getEquipmentDomSignature(eqbList) {
        return Array.from(eqbList, (eqb, index) => {
            ...
            return [
                index,
                getEquipmentSlotId(eqb),
                eqb.className,
                equipDiv?.dataset?.eid || '',
                equipDiv?.getAttribute('onmouseover') || '',
                equipDiv?.textContent?.trim() || '',
                infoSpan?.textContent?.trim() || '',
            ].join('\u001f');
        }).join('\u001e');
    }
    ...
    function getEquipmentData(force = false) {
        const slots = [];
        const eqbList = document.querySelectorAll('#eqsb .eqb');
        const signature = getEquipmentDomSignature(eqbList);
        if (!force && equipmentDataCache && equipmentDataCache.signature === signature) {
            return cloneEquipmentSlots(equipmentDataCache.slots);
        }
        eqbList.forEach((eqb, index) => {
            ...
            if (counterResistCharmBonus <= 0 && available && eid) {
                ...
                if (visibleCharmBonus > 0) {
                    ...
                    const cachedCharmInfo = CHARM_INFO_CACHE.get(eid);
                    if (Array.isArray(cachedCharmInfo)) {
                        counterResistCharmBonus = getCounterResistCharmBonusFromInfo(cachedCharmInfo);
                    } else if (CHARM_INFO_STATUS.get(eid) !== 'empty') {
                        counterResistCharmDataPending = true;
                        requestEquipmentCharmInfo(eid, info => {
                            const bonus = getCounterResistCharmBonusFromInfo(info);
                            if (activeUpgradeSimulation) {
                                ...
                                if (activeSlot) {
                                    activeSlot.counterResistCharmBonus = bonus;
                                    if (bonus > 0) normalizeCounterResistCharmData(activeSlot, bonus);
                                    else activeSlot.counterResistCharmDataPending = false;
                                }
                                if (activeUpgradeSimulation.refreshEquipmentData) {
                                    activeUpgradeSimulation.refreshEquipmentData();
                                }
        ...
        equipmentDataCache = {
            signature,
            slots: cloneEquipmentSlots(orderedSlots),
        };
        return cloneEquipmentSlots(orderedSlots);
    }
    ...
    const slotAttackCharmsCache = new WeakMap();
    ...
    function getSlotAttackCharms(slot, index) {
        ...
        const weaponClass = resolveWeaponClass(slot);
        const cached = slotAttackCharmsCache.get(slot);
        if (cached && cached.list === list && cached.key === key && cached.weaponClass === weaponClass) {
            return cached.value;
        }
        const defenseSlot = key === 'main' ? false : key === 'off' ? weaponClass === 'shield' : true;
        const value = (list || []).filter(charm => {
            const def = charm && CHARM_DEFS[charm.type];
            return def && (defenseSlot ? def.category === 'defense' : def.category === 'attack');
        });
        slotAttackCharmsCache.set(slot, { list, key, weaponClass, value });
        return value;
    }
    ...
    let playerLevelCacheText = '';
    let playerLevelCacheValue = 1;

    function getPlayerLevel() {
        const el = document.querySelector('#level_readout .fc4');
        if (!el) return 1;
        const text = el.textContent || '';
        if (el === playerLevelCacheElement && text === playerLevelCacheText) {
            return playerLevelCacheValue;
        }
        const match = text.match(/Lv\.(\d+)/);
        playerLevelCacheElement = el;
        playerLevelCacheText = text;
            ...
            if (maccCheckStatsObserverTarget !== scrollable) {
                ...
                maccCheckStatsObserver = new MutationObserver(mutations => {
                    const affectsStats = mutations.some(mutation => {
                        ...
                        return nodes.length === 0 || nodes.some(node => !node.classList?.contains?.('hv-panel-increment'));
                    });
  ...
  function buildUpgradePanel(equipSlots) {
    const buildToken = ++upgradePanelBuildToken;
    const cachedBaseStats = gBaseStatsFetched && gBasePrimaryStats
      ? gBasePrimaryStats
      : null;
    _buildPanel(equipSlots, cachedBaseStats);
    if (cachedBaseStats) return;
    fetchBasePrimaryStats(function(baseStats) {
      if (buildToken !== upgradePanelBuildToken || !activeUpgradeSimulation || !upgradePanel?.isConnected) return;
      if (!baseStats) {
        alert('无法读取基础属性，请检查网络或刷新页面。蓝色增量可能不准确。');
      }
      if (!baseStats) return;
      activeUpgradeSimulation.baseStats = baseStats;
      activeUpgradeSimulation.refreshDerived?.();
    });
  }
  ...
  function _buildPanel(equipSlots, baseStats) {
    ...
    const closePanel = () => {
      upgradePanelBuildToken++;
      if (simulationRefreshTimer !== null) {
        window.clearTimeout(simulationRefreshTimer);
    ...
    refreshCharmBtn.onclick = async function() {
      if (refreshCharmBtn.disabled) return;
      const previousValue = refreshCharmBtn.value;
      refreshCharmBtn.disabled = true;
      ...
      try {
        ...
        showToast(`护符刷新失败：${error.message || '读取失败'}`, 'warn');
      } finally {
        refreshCharmBtn.disabled = false;
        refreshCharmBtn.value = previousValue;
      }
    };
    btnContainer.append(resetBtn, materialBtn, refreshCharmBtn);
    ...
    activeUpgradeSimulation = {
      ...
      refreshDerived: scheduleSimulationRefresh,
    };
    activeUpgradeCRBaseline = Number.isFinite(actualCR) ? actualCR : null;
    const playerLevel = getPlayerLevel();
    let simulationRefreshTimer = null;

    function flushSimulationRefresh() {
      simulationRefreshTimer = null;
      if (!activeUpgradeSimulation || activeUpgradeSimulation.equipSlots !== equipSlots) return;
      applyIncrements(equipSlots, state);
      refreshMaterialPlan();
      notifyMaccSimulationChanged();
    }
    ...
    function scheduleSimulationRefresh() {
      if (simulationRefreshTimer !== null) return;
      simulationRefreshTimer = window.setTimeout(flushSimulationRefresh, 0);
    }
    ...
    function refreshEquipmentData() {
      ...
      const latestSlots = getEquipmentData(true);
    ...
    function applyIncrements(equipSlots, state) {
      ...
      const effectiveBaseStats = activeUpgradeSimulation?.baseStats || baseStats;
      if (effectiveBaseStats) {
        ...
        derivedIncrements = calculateDerivedIncrements(effectiveBaseStats, equipInit, equipCurr, playerLevel, tankSettings);
      } else {
      }
  ...
  function removeUpgradeButton() {
    ...
    if (equipmentMouseoverHandler) {
      document.removeEventListener('mouseover', equipmentMouseoverHandler, true);
      equipmentMouseoverHandler = null;
    }
    initialized = false;
  }
  ...
  function addUpgradeButton() {
    ...
    btn.onclick = async function() {
      ...
      btn.disabled = true;
      try {
        buildUpgradePanel(equipData);
        btn.disabled = false;
        const charmDataPromise = waitForEquipmentCharmData(equipData, false);
        const charmDataTimeout = new Promise(resolve => {
          window.setTimeout(() => resolve(equipData), 1500);
        });
        void Promise.race([charmDataPromise, charmDataTimeout]).catch(() => {});
      } finally {
        btn.disabled = false;
      }
    };
  ...
  let hvutRowSyncStopTimer = null;

  function stopHvutRowSync() {
    if (hvutRowSyncStopTimer !== null) {
      window.clearTimeout(hvutRowSyncStopTimer);
      hvutRowSyncStopTimer = null;
    }
  ...
  function startHvutRowSync() {
    ...
    hvutRowSyncStopTimer = window.setTimeout(() => {
      hvutRowSyncStopTimer = null;
      stopHvutRowSync();
    }, 15000);
  }
  ...
  let equipmentMouseoverHandler = null;

  function removeUpgradePanel() {
    upgradePanelBuildToken++;
    if (upgradePanel) {
      upgradePanel.remove();
      upgradePanel = null;
  ...
  function init() {
    ...
    initialized = true;
    equipmentMouseoverHandler = event => {
...
(function () {
  ...
  function log(message) {
    detailedLog += `${message}\n`;
    if (!logRenderSuspended) renderLog();
  }
  ...
  function applyFusion(donor, writeLog = true, persist = true) {
  ...
  function applyFusionPlan(donors) {
    ...
    try {
      for (const donor of donors) {
        if (!donor || !state.allEquips.some((equip) => equip.id === donor.id && !equip.used)) continue;
        applyFusion(donor, true, false);
      }
    } finally {
      logRenderSuspended = previousSuspended;
    }
    if (!previousSuspended) renderLog();
  }
  ...
  async function doBeamFusion() {
    ...
    applyFusionPlan(result.path.map((donorIndex) => donors[donorIndex]).filter(Boolean));
  ...
  async function doMinimumFusion() {
    ...
    saveState();
    applyFusionPlan(path.map((donorIndex) => donors[donorIndex]).filter(Boolean));
